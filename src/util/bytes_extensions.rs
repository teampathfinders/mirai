...
pub trait ReadAddress : Buf {
    fn get_addr(&mut self) -> VexResult<SocketAddr> {
        ...
        let ip_addr = match ip_type {
            4 => {
                IpAddr::V4(Ipv4Addr::from(self.get_u32()))
            },
            6 => {
                IpAddr::V6(Ipv6Addr::from(self.get_u128()))
            },
            _ => {
                return Err(VexError::InvalidRequest(format!("Invalid IP type: {}", ip_type)))
            }
        };
...
pub trait WriteAddress : BufMut {
    fn put_addr(&mut self, addr: SocketAddr) where Self: Sized {
        match addr {
