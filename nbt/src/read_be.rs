...
pub fn deserialize_be(stream: &mut Bytes) -> VResult<Tag> {
    let (name, value) = Value::deserialize_tag_be(stream)?;

    Ok(Tag { name, value })
}

impl Value {
    fn deserialize_tag_be(stream: &mut Bytes) -> VResult<(String, Self)> {
        let tag_type = stream.get_u8();
        if tag_type == TAG_END {
            return Ok((String::new(), Self::End));
        }

        let name = Self::deserialize_name_be(stream);
        let value = Self::deserialize_value_be(stream, tag_type)?;

        Ok((name, value))
    }

    fn deserialize_name_be(stream: &mut Bytes) -> String {
    ...
    fn deserialize_value_be(stream: &mut Bytes, tag_type: u8) -> VResult<Self> {
        Ok(match tag_type {
            TAG_END => Self::End,
            TAG_BYTE => {
            ...
            TAG_LIST => {
                ...
                for _ in 0..length {
                    list.push(Self::deserialize_value_be(stream, list_type)?);
                }
            ...
            TAG_COMPOUND => {
                ...
                loop {
                    tag = Self::deserialize_tag_be(stream)?;
                    if tag.1 == Self::End {
                        break;
                    }
