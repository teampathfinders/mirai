...
impl WriteBatch {
    ...
    pub fn put<K, V>(&mut self, key: K, val: V) where K: AsRef<[u8]>, V: AsRef<[u8]> {
        let key = key.as_ref();
        let val = val.as_ref();
        ...
        unsafe {
            ffi::batch_put(
                self.ptr.as_ptr(), 
                key.as_ptr() as *const i8, key.len() as c_int, 
                val.as_ptr() as *const i8, val.len() as c_int
            );
        }
    }

    /// Adds a delete operation to the batch.
    pub fn delete<K>(&mut self, key: K) where K: AsRef<[u8]> {
        let key = key.as_ref();
