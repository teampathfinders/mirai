...
pub trait ReadExtensions: Buf {
    ...
    fn get_vec3f(&mut self) -> Vector<f32, 3> {
        let a = self.get_f32();
        let b = self.get_f32();
        let c = self.get_f32();

        Vector::<f32, 3>::from([a, b, c])
    }
...
pub trait WriteExtensions: BufMut + Sized {
    ...
    fn put_vec3f(&mut self, value: &Vector<f32, 3>) {
    ...
    fn put_vec2i(&mut self, value: &Vector<i32, 2>) {
    ...
    fn put_vec3i(&mut self, value: &Vector<i32, 3>) {
        let [a, b, c] = value.components();
